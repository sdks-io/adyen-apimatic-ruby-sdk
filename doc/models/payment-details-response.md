...
| `donation_token` | `String` | Optional | Donation Token containing payment details for Adyen Giving. |
| `fraud_result` | [`FraudResult1`](../../doc/models/fraud-result-1.md) | Optional | The fraud result properties of the payment. |
| `merchant_reference` | `String` | Optional | The reference used during the /payments request. |
...
| `psp_reference` | `String` | Optional | Adyen's 16-character string reference associated with the transaction/request. This value is globally unique; quote it when communicating with us about this request. |
...
| `three_ds_payment_data` | `String` | Optional | When non-empty, contains a value that you must submit to the `/payments/details` endpoint as `paymentData`. |
...
{
  "action": {
    "authorisationToken": "authorisationToken6",
