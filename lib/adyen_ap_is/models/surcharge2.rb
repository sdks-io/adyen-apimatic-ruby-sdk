# adyen_ap_is
#
# This file was automatically generated by APIMATIC v3.0 (
# https://www.apimatic.io ).

module AdyenApIs
  # The [surcharge](https://docs.adyen.com/online-payments/surcharge/) amount to
  # apply to the transaction, in [minor
  # units](https://docs.adyen.com/development-resources/currency-codes). When
  # you apply surcharge, include the surcharge in the `amount.value` field.
  # Review our [Surcharge compliance
  # guide](https://docs.adyen.com/development-resources/surcharge-compliance/)
  # to learn about how to comply with regulatory requirements when applying
  # surcharge.
  class Surcharge2 < BaseModel
    SKIP = Object.new
    private_constant :SKIP

    # The [surcharge](https://docs.adyen.com/online-payments/surcharge/) amount
    # to apply to the transaction, in [minor
    # units](https://docs.adyen.com/development-resources/currency-codes). When
    # you apply surcharge, include the surcharge in the `amount.value` field.
    # Review our [Surcharge compliance
    # guide](https://docs.adyen.com/development-resources/surcharge-compliance/)
    # to learn about how to comply with regulatory requirements when applying
    # surcharge.
    # @return [Integer]
    attr_accessor :value

    # A mapping from model property names to API property names.
    def self.names
      @_hash = {} if @_hash.nil?
      @_hash['value'] = 'value'
      @_hash
    end

    # An array for optional fields
    def self.optionals
      []
    end

    # An array for nullable fields
    def self.nullables
      []
    end

    def initialize(value:)
      @value = value
    end

    # Creates an instance of the object from a hash.
    def self.from_hash(hash)
      return nil unless hash

      # Extract variables from the hash.
      value = hash.key?('value') ? hash['value'] : nil

      # Create object from extracted values.
      Surcharge2.new(value: value)
    end

    # Validates an instance of the object from a given value.
    # @param [Surcharge2 | Hash] The value against the validation is performed.
    def self.validate(value)
      if value.instance_of? self
        return APIHelper.valid_type?(value.value,
                                     ->(val) { val.instance_of? Integer })
      end

      return false unless value.instance_of? Hash

      APIHelper.valid_type?(value['value'],
                            ->(val) { val.instance_of? Integer })
    end

    # Provides a human-readable string representation of the object.
    def to_s
      class_name = self.class.name.split('::').last
      "<#{class_name} value: #{@value}>"
    end

    # Provides a debugging-friendly string with detailed object information.
    def inspect
      class_name = self.class.name.split('::').last
      "<#{class_name} value: #{@value.inspect}>"
    end
  end
end
