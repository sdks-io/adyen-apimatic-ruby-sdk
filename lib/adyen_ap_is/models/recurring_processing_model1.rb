...
module AdyenApIs
  ...
  # payment details. Allowed values: * `Subscription` – A transaction for a
  # fixed or variable amount, which follows a fixed schedule. * `CardOnFile` –
  # With a card-on-file (CoF) transaction, card details are stored to enable
  ...
  # a card-on-file transaction. * `UnscheduledCardOnFile` – An unscheduled
  # card-on-file (UCoF) transaction is a transaction that occurs on a non-fixed
  # schedule and/or have variable amounts. For example, automatic top-ups when a
  # cardholder's balance drops below a certain amount.
  class RecurringProcessingModel1
    RECURRING_PROCESSING_MODEL1 = [
    ...
    def self.validate(value)
      ...
      RECURRING_PROCESSING_MODEL1.include?(value)
    end
